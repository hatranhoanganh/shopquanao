...
export default function initModels(sequelize) {
  ...
  const category = _category.init(sequelize, DataTypes); // Khởi tạo mô hình category
...
// Quan hệ giữa product và gallery
product.belongsTo(gallery, { foreignKey: "id_gallery" });
...
order_product.belongsTo(product, { foreignKey: "id_product", as: "id_product_product" });
  ...
  return {
    user,
    category, // Đảm bảo category được trả về
    product,
    gallery,
    orders, 
    order_product,
